...
#define MovieProgressStep 0.0001f
...
- (void)playbackStalled:(NSNotification *)noti {
    //进入缓冲，3秒后播放
...
- (void)createAvPlayer {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        //后台线程处理AVPlayerItem
        AVPlayerItem *playerItem = [AVPlayerItem playerItemWithURL:(NSURL *) _movieURLList[0]];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            //注册检测视频加载状态的通知
            [self performSelector:@selector(delayMethod) withObject:nil afterDelay:7.0f];
...
- (void)countVideoLength {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        //第一个参数反应了检测的频率
        _timeObserver = [_player addPeriodicTimeObserverForInterval:CMTimeMake(3, 30)
                                                              queue:NULL
...
- (void)delayMethod {
    if ([[NetworkUtil me] reachable:YES]) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if ([object isKindOfClass:[AVPlayerItem class]] && [keyPath isEqualToString:@"status"]) {
        ...
        if (playerItem.status == AVPlayerStatusReadyToPlay) {
            ...
            //            [[NSNotificationCenter defaultCenter] postNotificationName:@"removeTrailerCover" object:nil];
...
- (void)popView {
    ...
    DLog(@"_player.status ===== %ld", (long) _player.status);

    [_player pause];

    DLog(@"After _player.status ===== %ld", (long) _player.status);
...
#define kLastTimeR @"lastTime"

            NSDate *lastShow = [[NSUserDefaults standardUserDefaults] objectForKey:kLastTimeR];
            if (!lastShow || [lastShow timeIntervalSinceNow] < -3) {

                //            NSLog(@"横向向右");
                _movieProgressSlider.value += 0.001;

                [[NSUserDefaults standardUserDefaults] setObject:[NSDate date] forKey:kLastTimeR];
            }

        } else {

#define kLastTimeL @"lastTime"

            NSDate *lastShow = [[NSUserDefaults standardUserDefaults] objectForKey:kLastTimeL];
            if (!lastShow || [lastShow timeIntervalSinceNow] < -3) {

                //            NSLog(@"横向向左");
                _movieProgressSlider.value -= 0.001;

                [[NSUserDefaults standardUserDefaults] setObject:[NSDate date] forKey:kLastTimeL];
            }
        }
...
- (void)addPlayRecordWithIdentifier:(NSString *)identifier progress:(CGFloat)progress {
    ...
    NSDictionary *dic = @{ MoviePlayerArchiveKey_identifier : identifier,
                           MoviePlayerArchiveKey_date : [NSDate date],
                           MoviePlayerArchiveKey_progress : @(progress) };
...
- (CGFloat)getProgressByIdentifier:(NSString *)identifier {
    ...
    [recardList enumerateObjectsWithOptions:NSEnumerationReverse
