...
- (void)dealloc {
    [[NSNotificationCenter defaultCenter] removeObserver:self taskType:TaskTypeForgetPassword];
}

#pragma mark Init views
- (UIScrollView *)holder {
    if (!_holder) {
        _holder = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 44 + self.contentPositionY, screentWith, screentContentHeight - 44)];
        _holder.backgroundColor = HEX(@"#F5F5F5");
        _holder.delaysContentTouches = NO;
        _holder.contentSize = CGSizeMake(screentWith, screentHeight - 43);
    }
    return _holder;
}
...
- (UIView *)inputHolder {
    if (!_inputHolder) {
        ...
        _inputHolder = [[UIView alloc] initWithFrame:CGRectMake(0, 15, screentWith, height)];
...
- (KKZTextField *)phoneNumberField {
    if (!_phoneNumberField) {
        CGFloat width = screentWith - 70 - 12;
...
- (KKZTextField *)validcodeField {
    if (!_validcodeField) {
        ...
        CGFloat width = screentWith - 70 - 90 - 20;
...
- (RoundCornersButton *)queryValidcodeButton {
    if (!_queryValidcodeButton) {
        CGFloat left = screentWith - 90 - 15;
        ...
        _queryValidcodeButton.rimColor = appDelegate.kkzBlue;
        _queryValidcodeButton.cornerNum = 4;
        _queryValidcodeButton.titleName = @"免费获取";
        _queryValidcodeButton.titleColor = appDelegate.kkzBlue;
...
- (void)queryValidcode {
    ...
    if ([phoneNumber length] != 11 || ![phoneNumber hasPrefix:@"1"]) {
        [UIAlertView showAlertView:@"请输入正确的手机号码" buttonText:@"确定" buttonTapped:nil];
        return;
    }
    ...
    [request requestResetPasswordValidcode:phoneNumber success:^{
        ...
        [UIAlertView showAlertView:@"验证码已经发出" buttonText:@"确定" buttonTapped:nil];
...
- (void)sendFindPassword {
    ...
    if (self.passwordField.text.length < 6) {
        [UIAlertView showAlertView:@"密码要求至少6位哦～" buttonText:@"确定" buttonTapped:nil];
        return;
    }
    ...
    [request resetPassword:self.phoneNumberField.text
            ...
            failure:^(NSError *_Nullable err) {
                ...
                [appDelegate hideIndicator];
                NSString *msg = [err.userInfo objectForKey:KKZRequestErrorMessageKey];
                if (msg.length > 0) {
                    [appDelegate showAlertViewForTitle:nil message:msg cancelButton:@"确定"];
                }
