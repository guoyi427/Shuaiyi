...
- (void)supportMovie {
    MovieTask *task = [[MovieTask alloc]
            initSupportMovieWithMovieid:self.movieId
                            andRelation:self.relation
                               finished:^(BOOL succeeded, NSDictionary *userInfo) {

                                   [self supportMovieFinished:userInfo status:succeeded];
                               }];
    [[TaskQueue sharedTaskQueue] addTaskToQueue:task];
}

- (void)supportMovieFinished:(NSDictionary *)userInfo status:(BOOL)succeeded {
    if (succeeded) {
