...
- (void)pushAnimateWithanimateTransition:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    UIViewController *fromVCOfAll = [transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    UIViewController *fromVC = [[fromVCOfAll childViewControllers] lastObject];
    if ([fromVC isKindOfClass:NSClassFromString(@"TicketViewController")]) {
        UIViewController *fromVC2 = [[fromVC childViewControllers] firstObject];
        fromVC = fromVC2;
    }
...
- (void)popAnimateWithanimateTransition:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    UIViewController *toVCOfAll = [transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    UIViewController *toVC = [[toVCOfAll childViewControllers] lastObject];
    if ([toVC isKindOfClass:NSClassFromString(@"TicketViewController")]) {
        UIViewController *toVC2 = [[toVC childViewControllers] firstObject];
        toVC = toVC2;
    }
    //起始位置
    CGRect originFrame = [fromVC.sf_targetView convertRect:fromVC.sf_targetView.bounds toView:fromVC.view];
    ...
    //移动的目标位置
    CGRect finishFrame = [toVC.sf_targetView convertRect:toVC.sf_targetView.bounds toView:toVCOfAll.view];
    ...
    toVC.view.frame = [transitionContext finalFrameForViewController:toVCOfAll];
    [containView addSubview:toVCOfAll.view];
