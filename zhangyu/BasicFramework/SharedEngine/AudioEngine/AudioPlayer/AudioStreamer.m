	...
	@synchronized(self)
	{
		...
		if (fileLength > 0 && seekByteOffset > 0)
		{
			CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Range"),
				(CFStringRef)[NSString stringWithFormat:@"bytes=%lld-%lld", seekByteOffset, fileLength]);
			discontinuous = YES;
		}
	...
	@synchronized(self)
	{
		if (state != AS_STARTING_FILE_THREAD)
		{
			if (state != AS_STOPPING &&
				state != AS_STOPPED)
			{
				DLog(@"### Not starting audio thread. State code is: %d", state);
			}
...
- (void)handleReadFromStream:(CFReadStreamRef)aStream eventType:(CFStreamEventType)eventType
{
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		if (discontinuous)
		{
			err = AudioFileStreamParseBytes(audioFileStream, (int32_t)length, bytes, kAudioFileStreamParseFlag_Discontinuity);
			if (err)
			{
		...
		{
			err = AudioFileStreamParseBytes(audioFileStream, (int32_t)length, bytes, 0);
			if (err)
			{
				[self failWithErrorCode:AS_FILE_STREAM_PARSE_BYTES_FAILED];
				return;
...
- (void)enqueueBuffer
{
	@synchronized(self)
	{
		...
		fillBuf->mAudioDataByteSize = (int32_t)bytesFilled;
		
		if (packetsFilled)
		{
			err = AudioQueueEnqueueBuffer(audioQueue, fillBuf, (int32_t)packetsFilled, packetDescs);
		}
		else
		{
