...
@interface MovieHotCommentController () {
...
@implementation MovieHotCommentController
- (void)dealloc {
...
- (void)loadMovieHotCommentList {
    ...
    if (self.cinemaId == 0) {
        ...
        [self requestMovieComment:currentPage];
    } else if (self.movieId == 0) {

        [self requestCinemaComment:currentPage];
    }
}

- (void)callMovieHotCommentApiDidSucceed:(NSArray *)responseData total:(NSInteger)total hasMore:(BOOL)hasMore {
    [appDelegate hideIndicator];
    showMoreFooterView.isLoading = NO;

    if (currentPage <= 1) {
        self.clubPosts = [[NSMutableArray alloc] initWithArray:responseData];
    } else {

        if ([responseData count] > 0) {

            [self.clubPosts addObjectsFromArray:responseData];
        }
    }
...
- (void)callMovieHotCommentApiDidFailed:(id)responseData total:(NSInteger)total {
    [appDelegate hideIndicator];
    showMoreFooterView.isLoading = NO;
...
/**
 ...
 @param page 页码
 */
- (void)requestMovieComment:(NSInteger)page {
...
/**
 ...
 @param page 页码
 */
- (void)requestCinemaComment:(NSInteger)page {
    showMoreFooterView.isLoading = YES;
    [appDelegate showIndicatorWithTitle:@"加载中"
                               animated:YES
                             fullScreen:NO
    ...
    ClubRequest *clubRequest = [ClubRequest new];
    [clubRequest requestCinemaHotCommentWithCinemaId:self.cinemaId
            currentPage:currentPage
            rowsNum:KKPageNum
            userGroup:self.userGroup
            success:^(NSArray *_Nullable hotComments, NSInteger total, BOOL hasMore) {
                ...
                if (currentPage <= 1) {
                    self.clubPosts = [[NSMutableArray alloc] initWithArray:hotComments];
                } else {

                    if (hotComments.count > 0) {

                        [self.clubPosts addObjectsFromArray:hotComments];
                    }
                }
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    if (scrollView.contentOffset.y <= -65.0f) {
        ...
        [self performSelector:@selector(loadMovieHotCommentList) withObject:nil afterDelay:0.2];
        //        }
    }
    //下拉刷新
