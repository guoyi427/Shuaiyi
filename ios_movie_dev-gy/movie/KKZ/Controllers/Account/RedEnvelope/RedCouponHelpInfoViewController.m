...
- (void)viewDidLoad {
    ...
    NSArray *infos = @[
        ...
        @{ @"title" : @"2、红包如何获得？",
           @"message" : @"用户通过微信等平台，获得章鱼电影送红包活动的红包金额奖励。\n获得红包后，章鱼电影新用户（首次使用手机注册的用户），注册完成后，便可使用现金红包。\n无论用户手机号码是否已被注册或手机曾用其他号码注册过章鱼电影账户，获得的红包将同步到参与红包活动的手机号上。" },
        @{ @"title" : @"3、红包如何使用？",
           @"message" : @"在订单支付页中，如果您的订单允许使用红包，章鱼电影会根据您的可用红包余额自动帮您计算出该订单最大的红包抵用金额，支付时所需现金相应扣减。暂不支持手动修改红包支付金额。\n若您的红包金额不足以支付订单金额，则不足部分仍需您继续使用其他方式予以支付。" },
