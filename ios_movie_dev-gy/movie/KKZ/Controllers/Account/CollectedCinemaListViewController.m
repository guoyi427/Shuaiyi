...
@interface CollectedCinemaListViewController ()
...
@implementation CollectedCinemaListViewController
...
- (void)handleTouchOnDetailAtRow:(NSInteger)row {
    Cinema *cinema = [collectList objectAtIndex:row];
    ...
    ticket.cinemaId = [NSNumber numberWithInt:cinema.cinemaId];
    ticket.cinemaCloseTicketTime = cinema.closeTicketTime;
...
- (void)cancelCollectCinema:(NSInteger)row {
    Cinema *cinema = [collectList objectAtIndex:row];
    FavoriteTask *task = [[FavoriteTask alloc]
            initDelFavCinema:cinema.cinemaId
                    finished:^(BOOL succeeded, NSDictionary *userInfo) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (!headerView) {
        headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, screentWith, 45)];
        headerView.backgroundColor = [UIColor whiteColor];
        UILabel *headerTitle = [[UILabel alloc] initWithFrame:CGRectMake((screentWith - 120) * 0.5, 14, 120, 14)];
        ...
        collectNumLabel = [[UILabel alloc] initWithFrame:CGRectMake((screentWith - 40) * 0.5, 14, 40, 14)];
