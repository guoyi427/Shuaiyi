...
#define decibel(amplitude) (20.0 * log10(absX(amplitude) / 32767.0))  //SPL=20 x log10[ p(e) / p(ref) ]　p(e)为待测声压有效值，p(ref)为参考声压。
...
//    _cropNormalView.backgroundColor = [UIColor blackColor];
    _cropProgressView = [[UIView alloc] init];
//    _cropProgressView.backgroundColor = [UIColor blackColor];
...
+ (void)renderWaveformInContext:(CGContextRef)context
                          asset:(AVAsset *)asset
                      withColor:(UIColor *)color
                        andSize:(CGSize)size
            antialiasingEnabled:(BOOL)antialiasingEnabled
{
    if (asset == nil) {
        return;
    }
    ...
    NSDictionary *outputSettingsDict = [[NSDictionary alloc] initWithObjectsAndKeys:
                                        ...
                                        [NSNumber numberWithBool:NO], AVLinearPCMIsNonInterleaved, //表明音频格式是否有间隔
                                        nil];
    ...
    while (reader.status == AVAssetReaderStatusReading) {
        ...
        if (sampleBufferRef) {
            ...
            for (int i = 0; i < sampleCount; i++) {
                Float32 sample = (Float32) *samples++; //4
                sample = decibel(sample); //-78
...
+ (UIImage*)generateWaveformImage:(AVAsset *)asset
                        withColor:(UIColor *)color
                          andSize:(CGSize)size
              antialiasingEnabled:(BOOL)antialiasingEnabled
{
    //创建画图的上下文,画布大小需要和屏幕的缩放比相乘
...
- (void)generateWaveforms
{
    ...
    if (self.generatedNormalImage == nil && self.asset) {
        self.generatedNormalImage = [SCWaveformView generateWaveformImage:self.asset
                                                                withColor:self.normalColor
                                                                  andSize:CGSizeMake(rect.size.width, rect.size.height)
                                                      antialiasingEnabled:self.antialiasingEnabled];
        _normalColorDirty = NO;
    }
    
    if (self.generatedNormalImage != nil) {
        if (_normalColorDirty) {
            self.generatedNormalImage = [SCWaveformView recolorizeImage:self.generatedNormalImage
                                                              withColor:self.normalColor];
            _normalColorDirty = NO;
        }
        
        if (_progressColorDirty || self.generatedProgressImage == nil) {
            self.generatedProgressImage = [SCWaveformView recolorizeImage:self.generatedNormalImage
