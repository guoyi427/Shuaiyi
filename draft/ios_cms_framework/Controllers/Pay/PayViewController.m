...
7.活动 自营活动--自营储值卡   系统方活动--自营/系统方储值卡   但是优惠价格是互斥的，优先使用活动的折扣价，如果使用了活动，储值卡不用查看折扣价了，可以支付
8.目前优惠券和会员卡都不能同时使用
*/
...
//    self.navigationController.navigationBar.translucent = YES;
    ...
    [_payMethodList addObject:@0];//会员卡
    [_payMethodList addObject:@1];//其他支付方式
...
- (void)setupUI{
    ...
    topHeaderView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kCommonScreenWidth, 732)];
...
//            make.height.equalTo(@(((kCommonScreenWidth-30)*40)/354+8));
    ...
    if (self.isHasPromotion) {
        ...
        eventTipLabel.textAlignment = NSTextAlignmentLeft;
        ...
        [eventTipLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(eventLabel.mas_right).offset(35);
            make.top.equalTo(@(0));
    ...
    couponLabel.text =@"";
    couponLabel.textColor = [UIColor colorWithHex:@"#ff9900"];
    ...
    [couponLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(@(15));
        make.left.equalTo(couponTipLabel.mas_right).offset(10);
        make.width.equalTo(@(100));
        make.height.equalTo(@(16));
    }];
    ...
    [topHeaderView setFrame:CGRectMake(0, 0, kCommonScreenWidth, positionY)];
    [payTableView setTableHeaderView:topHeaderView];
...
//    [selectSeatsLabel setAttributedText:seatInfoAttibutedString];
    if (self.myOrder.secondSeatInfo.length>0) {
    ...
    selectSeatsLabel.text = [NSString stringWithFormat:@"%@\n%@", self.myOrder.firstSeatInfo, self.myOrder.secondSeatInfo];
    ...
    ticketUnitPriceLabel.text = self.myOrder.filmPriceDesc;
    ticketPriceLabel.text = [NSString stringWithFormat:@"小计:￥%@", self.myOrder.totalPriceTicket];
    serviceLabel.text = [NSString stringWithFormat:@"已包含服务费￥%.2f", [self.myOrder.orderTicket.serviceMoney floatValue]/100];//单位  分
...
//    float couponMoney;//兑换券价格
    ...
    self.moneyToPay = ([self.myOrder.totalPriceTicket floatValue]+self.productMoney) - (self.promotionMoney+self.couponMoney+self.discountMoney);
...
- (void)confirmOrderClick{
    ...
    if ((self.selectCouponsList.count > 0)&&(self.selectCouponsList.count < [self.myOrder.orderTicket.count integerValue])) {
        [[CIASAlertCancleView new] show:@"提示" message:[NSString stringWithFormat:@"您已锁定%ld个座位，兑换券使用不足购买影票数量，请选择%ld个座位下单。", (long)[self.myOrder.orderTicket.count integerValue], self.selectCouponsList.count] cancleTitle:@"重新选座" callback:^(BOOL confirm) {
            if (!confirm) {
                [self deleteOrder];
            }
        }];
...
- (void)doPayOrder{
    ...
    if (self.selectedSection==1) {
        ...
        if (self.selectProductIds.length>0) {
            pagrams = [NSDictionary dictionaryWithObjectsAndKeys:self.orderNo,@"orderCode",ciasTenantId,@"tenantId", @"9",@"payType",  USER_CINEMAID,@"cinemaId",  self.selectProductIds.length <= 0? @"":self.selectProductIds,@"couponStr",self.selectCouponsString.length>0?self.selectCouponsString:@"",@"coupons", self.productType?@"2":@"1",@"goodsType", nil];
        }//goodsType;  1为系统方；2为自建
...
- (void)payOrderWithVipCard:(NSString *)codeStr{
    ...
    __weak __typeof(self) weakSelf = self;
    NSDictionary *pagrams = [NSDictionary dictionaryWithObjectsAndKeys:self.orderNo,@"orderCode",self.selectVipCardNo,@"cardNo", codeStr, @"code", nil];
...
//            VipCard_Store_WRITE(self.selectVipCardNo);
        ...
        for (UITextField *tf in _codeView.pzxView.textFieldArray) {
...
- (void)refreshPayOrderDetail{
    ...
    [request requestOrderDetailParams:pagrams success:^(id _Nullable data) {
         ...
         */
//        weakSelf.myOrder.orderMain.status = @4;//调页面用
        //主线程刷新，防止闪烁
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)requestOrderPay:(NSDictionary *)pagrams withPayType:(NSInteger)payType{
    ...
    [request requestPayOrderParams:pagrams success:^(NSDictionary *_Nullable data) {
        [[UIConstants sharedDataEngine] stopLoadingAnimation];
        if (payType==9) {
            if (self.couponMoney>=(self.productMoney+[self.myOrder.totalPriceTicket floatValue])) {
                ...
                ctr.totalMoney = [NSString stringWithFormat:@"%.2f", self.moneyToPay];
...
- (void)addCouponBtnClick {
    ...
    [[BindingCouponControl instance] showWithOrderNo:_orderNo complete:^(){
        
        weakSelf.showCouponList = true;
        [weakSelf requestMyCouponList:NO];
    }];
}
...
//获取储值卡列表
- (void)requestStoreVipCardListWithShowLog:(BOOL)isShow {
...
- (void)requestProductList{
    ...
    [requtest requestProductListParams:nil success:^(NSDictionary * _Nullable data) {
        ...
        if (self.isHasProduct) {
            if (self.selectCouponsList.count) {
        ...
        [self requestMyCouponList:YES];
        [self requestStoreVipCardListWithShowLog:NO];
        ...
        if (self.promotionList.count>0) {
            ...
            for (int i=0; i<self.promotionList.count; i++) {
                ...
                if ([self.selectActivityId isEqualToString:activity.activityId]) {
                    ...
                    self.selectActivityType = [NSString stringWithFormat:@"%@", activity.payLimit];//0-不限，1-会员，2-非会员
                }else{
...
- (void)requestUseActivity{
    ...
    if (self.promotionList.count>0) {
        ...
        NSDictionary *pagrams = [NSDictionary dictionaryWithObjectsAndKeys:self.orderNo,@"orderCode",self.selectActivityId,@"activityId", self.selectActivityType, @"activityType", nil];
        [[UIConstants sharedDataEngine] loadingAnimation];
...
//    couponArrowImageView.hidden = YES;
    ...
    [requtest requestMyCouponListParams:pagrams success:^(NSArray * _Nullable data) {
...
- (void)checkVipCardDiscount{
    ...
    [[UIConstants sharedDataEngine] loadingAnimation];
    //self.selectProductIds,@"goodsIds",
    NSDictionary *pagrams = [NSDictionary dictionaryWithObjectsAndKeys:self.orderNo,@"orderCode", aCard.cardNo,@"cardNo",
  nil];
...
- (void)getVipCardBalance{
    [[UIConstants sharedDataEngine] loadingAnimation];
    NSDictionary *pagrams = [NSDictionary dictionaryWithObjectsAndKeys:self.selectVipCardNo,@"cardNo",
                             nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView==payTableView) {
        if (indexPath.section==0) {
            //第三次互斥，券不能和储值卡列表同时使用
            if (self.selectCouponsList.count>0) {
                [CIASPublicUtility showAlertViewForTitle:@"" message:@"不可与兑换券同时使用" cancelButton:@"知道了"];
                return;
            }
            self.lastVipCardTitle = self.vipCardTitle;
            self.lastSelectVipCardNo = self.selectVipCardNo;
...
//                }
        ...
        if (self.selectVipCardNo.length>0) {
            [CIASPublicUtility showAlertViewForTitle:@"" message:@"不可与会员卡同时使用" cancelButton:@"知道了"];
            return;
        }
...
//                [couponListTable reloadData];
            ...
            if (self.selectCouponsList.count >= [self.myOrder.orderTicket.count integerValue]) {
                [CIASPublicUtility showAlertViewForTitle:@"" message:[NSString stringWithFormat:@"您已锁定%ld个座位，兑换券使用已超出购买影票数量。", [self.myOrder.orderTicket.count integerValue]] cancelButton:@"知道了"];
                return;
            }
...
- (void)showViewListWithIndex:(NSInteger)section{
    ...
    if (section==0) {
        if (self.myCardList.count == 0) {
            [self requestStoreVipCardListWithShowLog:YES];
            return;
        }
        selectedNum = -1;
        ...
        if ([self.selectActivityType isEqualToString:@"2"]) {
            self.vipCardTitle = @"不支持使用";
            [payTableView reloadData];
            [CIASPublicUtility showAlertViewForTitle:@"" message:@"当前选择的活动只支持在线支付" cancelButton:@"知道了"];
            return;
        }
        //第三次互斥，券不能和储值卡列表同时使用
        if (self.selectCouponsList.count>0) {
        ...
        if ([self.selectActivityType isEqualToString:@"1"]) {
            [CIASPublicUtility showAlertViewForTitle:@"" message:@"当前选择的活动只支持会员卡支付" cancelButton:@"知道了"];
            return;
...
- (void)PromotionListViewDidCell:(NSInteger)selectedIndex withData:(NSDictionary *)promotionDict{
    ...
    if (self.promotionList.count) {
        if (self.selectedPromotionIndex==-1) {
            ...
            //活动和储值卡  价格互斥 如果使用了活动的折扣价，就不需要查询储值卡的活动价了，如果取消活动，并且用户选择了储值卡就要查询折扣价了
            if (self.selectVipCardNo.length>0) {
                [self checkVipCardDiscount];
            }
        }else{
            ...
            if (self.selectVipCardNo.length>0) {
                if ([self.selectActivityType isEqualToString:@"2"]) {
                    ...
                    [payTableView reloadData];
                }else if (self.promotionType && !self.storeVipCardType) {
                    //卖品和储值卡类型不匹配
                    self.selectVipCardNo=@"";
                    selectedNum = -1;
                    self.discountMoney = 0.0f;
            ...
            if ([self.selectActivityType isEqualToString:@"0"] || [self.selectActivityType isEqualToString:@"2"]) {
                self.selectedSection = 1;
                [payTableView reloadData];
            }else if ([self.selectActivityType isEqualToString:@"1"]){
                self.selectedSection = -1;
                [payTableView reloadData];
            }
...
- (CodeView *)codeView {
    if (!_codeView) {
        _codeView = [[CodeView alloc] initWithFrame:CGRectMake((kCommonScreenWidth - 286)/2, (kCommonScreenHeight - 230 -49)/2, 286, 250) delegate:self andOrderNo:self.orderNo];
    }
    return _codeView;
}
...
- (void)ProductListViewDicts:(NSArray *)dictArray{
    ...
    /*
活动优惠价格 和 储值卡折扣价 对卖品价格没有影响    但是逻辑有互斥的关系
