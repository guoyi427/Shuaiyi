...
@interface CityListNewViewController () <CityCellDelegate>
/**
 *  城市列表
 */
...
//    locationBtn.frame = CGRectMake(kCommonScreenWidth - 70, 4, 60, 38);
...
//    [locationBtn addTarget:self action:@selector(locationCityClick) forControlEvents:UIControlEventTouchUpInside];
    ...
    UIBarButtonItem *locationBtn = [[UIBarButtonItem alloc] initWithTitle:@"重新定位" style:UIBarButtonItemStylePlain target:self action:@selector(locationCityClick)];
...
- (void)loadTable {
    if (!cityTable) {
        cityTable = [[UITableView alloc] initWithFrame:CGRectMake(6, 0, kCommonScreenWidth - 6 * 2, kCommonScreenHeight - 44)
        ...
        refreshHeaderView = [[EGORefreshTableHeaderView alloc]
                initWithFrame:CGRectMake(0, -cityTable.bounds.size.height, kCommonScreenWidth, cityTable.bounds.size.height)];
 ...
 */
- (void)addTableNotice {
    nodataView = [[NoDataViewY alloc] initWithFrame:CGRectMake(0, kCommonScreenHeight * 0.5 - 120, kCommonScreenWidth, 120)];
    nodataView.alertLabelText = @"获取城市列表失败";
    noAlertView = [[AlertViewY alloc] initWithFrame:CGRectMake(0, kCommonScreenHeight * 0.5 - 120, kCommonScreenWidth, 120)];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (!headerView) {
        @synchronized([CityListNewViewController class]) {
            if (!headerView) {
                headerView = [[CityCellHeaderView alloc] initWithFrame:CGRectMake(0, 0, kCommonScreenWidth, 35)];
...
//        [UIAlertView showAlertView:@"请先选择城市" buttonText:@"确定" buttonTapped:nil];
        [[CIASAlertCancleView new] show:@"请先选择城市" message:nil cancleTitle:@"确定" callback:^(BOOL confirm) {
        }];
    } else {
