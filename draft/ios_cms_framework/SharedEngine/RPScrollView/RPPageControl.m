...
- (void)p_initialize {
    ...
    _currentIndicatorTintColor = [UIColor colorWithHex:[UIConstants sharedDataEngine].lumpColor];
...
- (void)setNumberOfPages:(NSInteger)numberOfPages {
    ...
    if (numberOfPages == 1) {
        self.systemPageControl.hidden = YES;
    } else {
        self.systemPageControl.hidden = NO;
    }
...
-(void)dealloc {
    if (self.systemPageControl) {
        [self.systemPageControl removeFromSuperview];
        self.systemPageControl = nil;
    }
}
