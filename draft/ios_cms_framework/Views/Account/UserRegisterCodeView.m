...
@interface UserRegisterCodeView ()
{
...
@implementation UserRegisterCodeView
- (void)dealloc
{
...
- (id)initWithFrame:(CGRect)frame delegate:(id<UserRegisterCodeViewDelegate>)aDelegate {
    if (self = [super initWithFrame:frame]) {
        ...
        [backBtnOfRegister addTarget:self action:@selector(backBtnOfRegisterClick:) forControlEvents:UIControlEventTouchUpInside];
        ...
        CGSize registerStrSize = [KKZTextUtility measureText:@"绑定手机号" size:CGSizeMake(500, 500) font:[UIFont systemFontOfSize:30]];
        ...
        [loginBtnOfRegister addTarget:self action:@selector(loginBtnOfRegisterCodeViewClick:) forControlEvents:UIControlEventTouchUpInside];
        //        [loginBtnOfRegister2 addTarget:self action:@selector(loginBtnOfRegisterCodeViewClick:) forControlEvents:UIControlEventTouchUpInside];
        ...
        NSString *phoneNum = USER_DEFAULT_REGISTERPHONE;
        ...
        [gotoRegisterBtn addTarget:self action:@selector(gotoRegisterBtnClickOfRegisterView:) forControlEvents:UIControlEventTouchUpInside];
...
- (void) backBtnOfRegisterClick:(id)sender {
    if (registerCodeTimer && registerCodeTimer.isValid) {
        [registerCodeTimer invalidate];
        registerCodeTimer = nil;
    }

    if (self.delegate && [self.delegate respondsToSelector:@selector(backBtnClickOfGetCodeView)]) {
        [self.delegate backBtnClickOfGetCodeView];
    }
}
//上一步
- (void) loginBtnOfRegisterCodeViewClick:(id)sender {
    if (registerCodeTimer && registerCodeTimer.isValid) {
        [registerCodeTimer invalidate];
        registerCodeTimer = nil;
    }
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(gotoLoginViewOfGetCodeViewButtonClick)]) {
        [self.delegate gotoLoginViewOfGetCodeViewButtonClick];
    }
}
...
//下一步
- (void) gotoRegisterBtnClickOfRegisterView:(id)sender {
    if (codeField.text.length < 7) {
        //        [[CIASAlertCancleView new] show:@"请填写短信里的验证码" message:@"如果验证码不正确，您将无法注册" cancleTitle:@"重新填写" callback:^(BOOL confirm) {
        //        }];
        return;
    }
    if (registerCodeTimer && registerCodeTimer.isValid) {
    ...
    if (self.wechatBindPhoneState) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(wechatBindRegistPhoneWithAccount:codeStr:)]) {
            [self.delegate wechatBindRegistPhoneWithAccount:USER_DEFAULT_REGISTERPHONE codeStr:[codeField.text stringByReplacingOccurrencesOfString:@" " withString:@""]];
        }
    } else {
        if (self.delegate && [self.delegate respondsToSelector:@selector(userRegisterButtonClickWithAccount:codeStr:)]) {
            [self.delegate userRegisterButtonClickWithAccount:USER_DEFAULT_REGISTERPHONE codeStr:[codeField.text stringByReplacingOccurrencesOfString:@" " withString:@""]];
        }
    }
}
