...
@interface UserRegisterPhoneView ()
{
...
@implementation UserRegisterPhoneView
...
- (id)initWithFrame:(CGRect)frame delegate:(id<UserRegisterPhoneViewDelegate>)aDelegate {
    if (self = [super initWithFrame:frame]) {
        ...
        [loginBtnOfRegister addTarget:self action:@selector(loginBtnOfRegisterClick:) forControlEvents:UIControlEventTouchUpInside];
        [loginBtnOfRegister2 addTarget:self action:@selector(loginBtnOfRegisterClick:) forControlEvents:UIControlEventTouchUpInside];
...
//            make.size.mas_equalTo(CGSizeMake(loginStrSize.width + 5, loginStrSize.height));
        ...
        [phoneImageViewOfRegister mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(bgView.mas_left).offset(30);
            make.top.equalTo(phoneTipsLabelOfRegister.mas_bottom).offset(50);
...
//        [getCodeBtn setTitleColor:[UIColor colorWithHex:[UIConstants sharedDataEngine].btnCharacterColor] forState:UIControlStateNormal];
        ...
        NSString *strOfTips = @"错误 已存在相关此手机号的账户，请尝试登录";
        ...
        [AttributedStr addAttribute:NSForegroundColorAttributeName
                              ...
                              range:NSMakeRange(3, strOfTips.length-3)];
...
- (void) backBtnOfRegisterClick:(id)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(backBtnClickOfRegisterView)]) {
        [self.delegate backBtnClickOfRegisterView];
    }
}
//   登录/已有账号  返回到登录界面
- (void) loginBtnOfRegisterClick:(id)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(gotoLoginViewButtonClick)]) {
        [self.delegate gotoLoginViewButtonClick];
    }
}
...
//        }];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(userGetCodeButtonClickWithAccount:)]) {
        [self.delegate userGetCodeButtonClickWithAccount:[phoneField.text stringByReplacingOccurrencesOfString:@" " withString:@""]];
    }
}
...
- (void)updateStateToBind:(BOOL)isWechatBind {
    ...
    if (isWechatBind) {
        phoneTipsLabelOfRegister.text = @"还没有账号？";
        ...
        [loginBtnOfRegister setTitle:@"已有账号" forState:UIControlStateNormal];
    } else {
        phoneTipsLabelOfRegister.text = @"您的手机号？";
        ...
        [loginBtnOfRegister setTitle:@"登录" forState:UIControlStateNormal];
    }
}
...
- (void)updateErrorStatus:(BOOL)error {
    if (error) {
        phoneField.rightView = nil;
        UIImageView *rightImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
        rightImageView.image = [UIImage imageNamed:@"register_tan"];
        rightImageView.contentMode = UIViewContentModeScaleAspectFit;
        phoneField.rightView = rightImageView;
        phoneField.rightViewMode = UITextFieldViewModeAlways;
    } else if (phoneField.text.length == 13) {
        phoneField.rightView = nil;
        UIImageView *rightImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
        rightImageView.image = [UIImage imageNamed:@"login_right"];
        rightImageView.contentMode = UIViewContentModeScaleAspectFit;
