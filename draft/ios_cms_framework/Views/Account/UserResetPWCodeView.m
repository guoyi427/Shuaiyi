...
@implementation UserResetPWCodeView
...
- (id)initWithFrame:(CGRect)frame delegate:(id<UserResetPWCodeViewDelegate>)aDelegate {
    if (self = [super initWithFrame:frame]) {
        ...
        NSString *phoneNum = USER_DEFAULT_RESETPWPHONE;
...
//        gotoRegisterBtn.backgroundColor = [UIColor colorWithHex:[UIConstants sharedDataEngine].btnColor];
        ...
        [gotoRegisterBtn addTarget:self action:@selector(gotoBtnClickOfResetPWView:) forControlEvents:UIControlEventTouchUpInside];
        ...
        //  底部黄色进度条
        UIView *bottomYellowProgressBar = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(frame)-4, kCommonScreenWidth/3.0*2, 4)];
...
- (void) backBtnOfPWClick:(id)sender {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(backBtnClickOfResetPWCodeView)]) {
        [self.delegate backBtnClickOfResetPWCodeView];
    }
}
...
- (void) loginBtnOfRegisterCodeViewClick:(id)sender {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(backBtnClickTOResetPWPhoneView)]) {
        [self.delegate backBtnClickTOResetPWPhoneView];
    }
}

//下一步
- (void) gotoBtnClickOfResetPWView:(id)sender {
...
//        }];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(userOfResetPWCodeButtonClickWithAccount:codeStr:)]) {
        [self.delegate userOfResetPWCodeButtonClickWithAccount:USER_DEFAULT_RESETPWPHONE codeStr:[codeField.text stringByReplacingOccurrencesOfString:@" " withString:@""]];
    }
}
