...
@implementation UserForgetResetPWView
...
- (id)initWithFrame:(CGRect)frame delegate:(id<UserForgetResetPWViewDelegate>)aDelegate {
    if (self = [super initWithFrame:frame]) {
        ...
        [backBtnOfRegister addTarget:self action:@selector(backBtnOfForgetPWClick:) forControlEvents:UIControlEventTouchUpInside];
        ...
        //MARK: 登录标题
        NSString *phoneTipsStr = @"输入至少8位的数字或字母";
        ...
        CGSize phoneTipsStrSize2 = [KKZTextUtility measureText:phoneTipsStr2 size:CGSizeMake(500, 500) font:[UIFont systemFontOfSize:16]];
        ...
        [passwordField addTarget:self action:@selector(passwordTextFieldDidChange) forControlEvents:UIControlEventEditingChanged];
...
- (void) gotoBtnClickOfForgetPWView:(id)sender {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(userResetPWButtonClickWithAccount:passWord:)]) {
        [self.delegate userResetPWButtonClickWithAccount:USER_DEFAULT_FORGETPWPHONE passWord:passwordField.text];
    }
}


- (void)backBtnOfForgetPWClick:(id)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(backBtnClickOfForgetPWResetPWView)]) {
        [self.delegate backBtnClickOfForgetPWResetPWView];
    }
}
