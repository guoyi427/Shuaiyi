...
@interface DottedLineBordaerView : UIView
...
@implementation DottedLineBordaerView
...
-(void)drawRect:(CGRect)rect
{
    ...
    if (_cropsImageToCircle) {
        self.border.path = [UIBezierPath bezierPathWithOvalInRect:self.bounds].CGPath;
    } else {
...
@interface CPCropperView ()<UIScrollViewDelegate>
@property (nonatomic, strong) DottedLineBordaerView *borderView;
...
- (void)setup {
    ...
    self.borderView = [[DottedLineBordaerView alloc] initWithFrame:self.cropMaskView.bounds];
...
- (void)setCropsImageToCircle:(BOOL)cropsImageToCircle {
    ...
    _cropsImageToCircle = cropsImageToCircle;
    _borderView.cropsImageToCircle = cropsImageToCircle;
    [self setNeedsLayout];
}
...
- (void)layoutSubviews {
    ...
    if(self.cropSize.width > 0 && self.cropSize.height > 0) {
        ...
        CGRect displayCropRect = CGRectMake(CGRectGetMidX(self.bounds) - self.displayCropSize.width/2.0f,
                                            CGRectGetMidY(self.bounds) - self.displayCropSize.height/2.0f,
                                            self.displayCropSize.width,
                                            self.displayCropSize.height);
...
- (void)updateScrollViewContentInset {
    ...
    self.scrollView.contentInset = UIEdgeInsetsMake(verticalInset + self.cropDisplayOffset.vertical,
                                                    horizontalInset + self.cropDisplayOffset.horizontal,
                                                    verticalInset - self.cropDisplayOffset.vertical,
                                                    horizontalInset - self.cropDisplayOffset.horizontal);
}
...
- (void)centerImageInScrollView:(UIScrollView *)scrollView {
    ...
    if (contentSizeWidth >= kCommonScreenWidth && scrollView.contentSize.height <= kCommonScreenWidth) {
        [scrollView setContentOffset:CGPointMake((contentSizeWidth-scrollView.bounds.size.width)/2.0, _scrollView.contentOffset.y)];
    } else {
        [scrollView setContentOffset:CGPointMake(_scrollView.contentOffset.x, (_scrollView.contentSize.height - scrollView.bounds.size.height)/2.0)];
    }
}
...
- (void)renderCroppedImage:(void (^)(UIImage *croppedImage, CGRect cropRect))completionBlock {
    ...
    BOOL cropToCircle = false;//self.cropsImageToCircle;    暂定剪切图片 圆形剪裁框 已经剪切正方形
