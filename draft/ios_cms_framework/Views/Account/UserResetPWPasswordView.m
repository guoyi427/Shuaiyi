...
@implementation UserResetPWPasswordView
...
- (id)initWithFrame:(CGRect)frame delegate:(id<UserResetPWPasswordViewDelegate>)aDelegate {
    if (self = [super initWithFrame:frame]) {
        ...
        [backBtnOfRegister addTarget:self action:@selector(backBtnOfResetPasswordClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)backBtnOfResetPasswordClick:(id)sender {
//
    if (self.delegate && [self.delegate respondsToSelector:@selector(backBtnClickOfResetPWPasswordView)]) {
        [self.delegate backBtnClickOfResetPWPasswordView];
    }
}
...
- (void) gotoBtnClickOfForgetPWView:(id)sender {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(userResetPWPasswordButtonClickWithAccount:passWord:)]) {
        [self.delegate userResetPWPasswordButtonClickWithAccount:USER_DEFAULT_RESETPWPHONE passWord:passwordField.text];
    }
}
