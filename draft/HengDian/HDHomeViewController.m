    ...
    if ([USER_CITY length] <= 0) {
        ...
        cityListCtr.selectCityBlock = ^(NSString *cityId){
            ...
            ctr.selectCinemaBlock = ^(NSString *cinemaId){
                NSString *cinemaStr = USER_CINEMA_SHORT_NAME;
                cinemaNameLabel.text = [NSString stringWithFormat:@"%@", ([cinemaStr length]&&(![cinemaStr isEqualToString:@"(null)"])&&(![cinemaStr isEqualToString:@"null"]))?USER_CINEMA_SHORT_NAME:@""];
...
- (void)requestProductList{
    ...
    [requtest requestProductListParams:nil success:^(NSDictionary * _Nullable data) {
        ...
        if (weakSelf.productList.count > 0) {
            ...
            [headerTableView setFrame:CGRectMake(0, 0, kCommonScreenWidth, (K_BANNER_HEIGHT+125+158+20+105+40+43))];
            if (weakSelf.homeOrderDic) {
                NSString *status = [weakSelf.homeOrderDic kkz_stringForKey:@"status"];
                if ([status isEqualToString:@"0"]) {
                    [headerTableView setFrame:CGRectMake(0, 0, kCommonScreenWidth, (K_BANNER_HEIGHT+125+158+20+105+40+43+55))];
                }
            }
            ...
            //  无商品，隐藏页面
            [headerTableView setFrame:CGRectMake(0, 0, kCommonScreenWidth, (K_BANNER_HEIGHT+125+158))];
            if (weakSelf.homeOrderDic) {
                NSString *status = [weakSelf.homeOrderDic kkz_stringForKey:@"status"];
                if ([status isEqualToString:@"0"]) {
                    [headerTableView setFrame:CGRectMake(0, 0, kCommonScreenWidth, (K_BANNER_HEIGHT+125+158+55))];
                }
            }
        ...
        [homeTableView setTableHeaderView:headerTableView];
...
- (void)setUpNavBar{
    UIView *bar = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kCommonScreenWidth, 64)];
    bar.backgroundColor = [UIColor whiteColor];
    
    cinemaNameView = [[UIView alloc] initWithFrame:CGRectMake(0, 20, kCommonScreenWidth, 44)];
    cinemaNameView.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:bar];
    [bar addSubview:cinemaNameView];
    
    //  logo
    UIImageView *logoImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"title_logo"]];
    [cinemaNameView addSubview:logoImageView];
    [logoImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(cinemaNameView);
    }];
    ...
    cinemaNameLabel.textColor = [UIColor colorWithHex:[UIConstants sharedDataEngine].lumpColor];
    ...
    [cinemaNameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(cinemaNameView);
        make.right.equalTo(cinemaNameView).offset(-8);
    ...
    arrowImageView.image = [UIImage imageNamed:@"title_location"];
    [cinemaNameView addSubview:arrowImageView];
    [arrowImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(cinemaNameLabel.mas_left);
        make.centerY.equalTo(cinemaNameLabel);
    }];
    ...
    [cinemaNameView addSubview:selectCinemaBtn];
    [selectCinemaBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(arrowImageView);
        make.right.equalTo(cinemaNameLabel);
        make.top.bottom.equalTo(cinemaNameView);
    }];
...
- (void)setUpUI{
    homeTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, kCommonScreenWidth, kCommonScreenHeight-49-64) style:UITableViewStylePlain];
...
- (void)changeCinemaSucceeded{
    ...
    NSString *cinemaStr = [USER_CINEMA_SHORT_NAME length]?USER_CINEMA_SHORT_NAME:USER_CINEMA_NAME;
    cinemaNameLabel.text = [NSString stringWithFormat:@"%@", ([cinemaStr length]&&(![cinemaStr isEqualToString:@"(null)"])&&(![cinemaStr isEqualToString:@"null"]))?cinemaStr:@""];
...
//        OrderDetailViewController *ctr = [[OrderDetailViewController alloc] init];
        //ctr.amovie = movie;
//        [self.navigationController pushViewController:ctr animated:YES];
        [CIASPublicUtility showAlertViewForTitle:@"" message:@"暂未开放单独购买方式，谢谢期待!" cancelButton:@"知道了"];
